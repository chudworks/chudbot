    ...
    R: BotRuntimeTypes + 'static,
{
    ...
    fn compose_agent_instruction_parts(
        ...
        if policy.final_reply_attach() {
            out.push_str("- Stored media assets can be checked with stat, resolved to a configured public URL with public_url, visually inspected with read, and explicitly attached to the final platform reply with attach. Use exact media://... stored URIs, legacy file://... stored URIs only when already present in context, user_avatar://... for cached user avatars, and guild_icon://... for the current guild icon; do not invent media://avatars/<user_id>.jpg paths. read only accepts verified stored image assets; attach accepts verified stored image or video assets. These tools never return file bytes and reject audio, PDFs, unknown MIME types, public URLs, and local filesystem paths. attach deduplicates with generated media already queued for the final reply.\n");
